    ...
    public void drawUp()
    {
        ...
        for (int c = 0; c < grid.GetLength(0); c++)
        {
            for (int d = 0; d < grid.GetLength(0); d++)
            {
                ...
                if (grid[c, d] == 'X')
                {
                    instance = (GameObject)Instantiate(Resources.Load("Wall"));
                }
                else  //else decide if trap or not
                {
                    instance = (GameObject)Instantiate(Resources.Load("Base"));
                }
                
                if (instance.name == "Base(Clone)")
                {
                    instance.transform.position = new Vector3(transform.position.x + c * 1f - 16.5f, transform.position.y - .5f, transform.position.z + d * 1 - 16.5f);
                } else
                {
                    instance.transform.position = new Vector3(transform.position.x + c * 1f - 16.5f, transform.position.y, transform.position.z + d * 1 - 16.5f);
                }

                instance.transform.parent = transform;
            }
        }
        var runner = (GameObject)Instantiate(Resources.Load("MazeRunner"));
        runner.transform.parent = transform;
        runner.transform.position = new Vector3(transform.position.x - 16f, transform.position.y + 1f, transform.position.z + 1f - 16.5f);
